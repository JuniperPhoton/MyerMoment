...
namespace MyerMomentUniversal.ViewModel
{
    public class StylesViewModel:ViewModelBase
    {
        ...
        private async Task<bool> DeleteStyleFile(string displayName)
        {
                ...
                var thumbFile = await StorageFileHandleHelper.TryGetFile(folder, displayName + ".png");
                if(thumbFile==null)
                {
                    return false;
                }
                var fullSizeFile = await StorageFileHandleHelper.TryGetFile(folder, displayName + "2.png");
